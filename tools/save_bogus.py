...
def save_lae(detectid):
	...
	save_file = os.path.join(basedir, f"radial_profiles/bogus_laes_skymask/bogus_2_{detectid}.dat")
...
for shotid in np.unique(complete_lae_tab["shotid"])[::-1]:
	...
	for detectid in laes_here["detectid"].data:
		...
		#done *= t >= t_oldest
		done *= os.path.exists(os.path.join(basedir, f"radial_profiles/bogus_laes_skymask/bogus_2_{detectid}.dat"))
