...
class SaveLike(likelihood.Likelihood):
	def initialize(self):

		self.def_wave = np.arange(3470, 5542, 2)
		
		dets_laes_all = ascii.read(basedir+ "intensity-mapping/tables/new_laes.tab" )#"lists/dets_laes.tab")
	...
	def load_from_shot(self):
		...
		for lae in dets_laes_all:
			...
			# save it
			self.save_radial_profile(lae["detectid"])

	def save_radial_profile(self, lae_id):
		tab = {"r": self.stardists[0], "flux": self.starflux[0], "sigma": self.starsigma[0]}
		ascii.write(tab, self.save_dir + f"lae_{lae_id}.dat", overwrite=True)
		print(f"Wrote to {self.save_dir}lae_{lae_id}.dat")
